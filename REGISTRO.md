...
Sigue el manual de marca de Edrink (Estudio Rosa, V.1 — 2026): negro y blanco como base, lima para
destacados y CTAs, cian para estados activos (aceptado, focus, radios/checkboxes) y magenta como
textura de fondo en los momentos "nocturnos" (el radar de despacho, el pie de página). Montserrat es
la única familia tipográfica, en los pesos que define el manual: ExtraBold para titulares y cifras
destacadas, Bold para rótulos y botones, Medium Italic para la bajada del logo ("bartender" junto al
wordmark EDRINK).

El panel de la botillería invierte la piel a modo diurno de trastienda: es el otro lado del
mostrador y debe verse como otro producto, con la misma paleta pero en versión positiva (fondo
claro).
...
**2026-09-04, cambio de ciudad y de red.** El cliente no opera en Santiago sino en Antofagasta, y no
tiene una red de ocho locales sino una sola botillería propia: **Edrink Antofagasta**, en Av.
Edmundo Pérez Zujovic 4764. Se reemplazó `lib/botillerias.ts` completo: un único registro en
`BOTILLERIAS`, comunas de Antofagasta en `SECTORES` (La Chimba, Centro, Los Arenales, Coviefi,
Bonilla, La Portada, Jardines del Sur) y `RADIOS_KM` pasó de `[3, 7, 15]` a `[15]`: con un solo
local no tiene sentido escalar el radio por etapas para "alcanzar otro local que sí conteste", así
que ahora el único radio representa la zona de cobertura de despacho. El resto del motor de
`lib/pedidos.ts` no cambió: es genérico sobre el número de botillerías, así que sigue funcionando
igual con una que con ocho. Se reescribió el texto de `ComoFunciona.tsx`, `SeguimientoPedido.tsx`,
...
- **Color** (`app/globals.css`): la paleta azul noche + naranja se reemplazó por negro `#0A0A0A` /
  blanco `#FFFFFF` de base, con tres acentos neón — lima `#DFF75C` (destacados y CTAs), cian
  `#10E2FF` (estados activos: aceptado, focus, radios/checkboxes) y magenta `#C400E1` (fondos y
  "branding nocturno"). Se mantuvieron los nombres de variable existentes donde el rol no cambiaba
  de sentido (`noche`, `hueso`, `bruma`, `tinta`); donde el nombre nombraba un color específico
  (`sol` → naranja) se migró el className completo a `lima` o `cyan` según el rol real, en vez de
  apuntar el nombre viejo a un color distinto. Se agregó `vino` como color funcional (no de marca)
  para errores y estados de rechazo, coherente con la familia neón.
- **Tipografía** (`app/layout.tsx`, `globals.css`): Archivo + Instrument Sans se reemplazaron por
  **Montserrat** única, como pide el manual, en los pesos Regular/Medium/Medium Italic/Bold/
  ExtraBold.
- **Textura de marca**: el manual define un fondo de ladrillo con degradé neón magenta-violeta para
  piezas nocturnas. No se contaba con ese asset, así que se aproximó con un degradé radial en CSS
  (`.textura-marca` en `globals.css`), aplicado al pie de página y al panel del radar de despacho —
  el momento más "nocturno" del flujo.
- **Wordmark**: `EDRINK` en mayúsculas (antes "edrink" en minúsculas) con la bajada "bartender" en
  Montserrat Medium Italic y cian, replicando el tratamiento tipográfico de "Click and brindis" del
  logo real.
- Se consolidó `Boton` a un solo variante primario (`lima`, antes existían `sol` y `lima` sin usar
  ambos) y se eliminó el variante `discreto`, que no se usaba en ningún lado.

Pendiente: el isologo real (el trazado a mano con líneas de velocidad) no se implementó como
gráfico — el manual solo llegó como PDF exportado y no se contaba con el archivo de logo (SVG/PNG).
El wordmark del sitio es una aproximación tipográfica fiel al manual, no el isologo real. Si el
