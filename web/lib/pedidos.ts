...
export function crearPedido(datos: {
  ...
  const pedido = conEvento(
    ...
    ahora,
    ofertas.length === 0
      ? 'Sin cobertura desde esta dirección'
      : datos.modo === 'manual'
        ? 'Aviso a la botillería, esperando respuesta desde el panel'
        : 'Aviso a la botillería',
  )
...
export function avanzar(p: Pedido, ahora: number): Pedido | null {
  ...
  if (x.estado === 'buscando') {
    ...
    if (!quedaAlguna) {
      ...
      return conEvento({ ...x, estado: 'sin_cobertura' }, ahora, 'La botillería no pudo tomarlo a tiempo')
    }
