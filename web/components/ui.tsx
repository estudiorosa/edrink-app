  ...
  return (
    <button
      ...
      className={[
        ...
        activo
          ? 'border-lima bg-lima text-noche'
...
type BotonProps = ButtonHTMLAttributes<HTMLButtonElement> & {
  variante?: 'lima' | 'contorno'
  tamano?: 'normal' | 'grande'
}

export function Boton({ variante = 'lima', tamano = 'normal', className = '', ...props }: BotonProps) {
  const estilos = {
    lima: 'bg-lima text-noche hover:bg-lima-hondo disabled:bg-noche-borde disabled:text-bruma',
...
export function Etiqueta({ children, tono = 'bruma' }: { children: ReactNode; tono?: 'bruma' | 'lima' }) {
  const color = {
