...
import { POR_ID, distanciaDe } from '@/lib/botillerias'
    ...
    <main className="mx-auto max-w-6xl px-5 py-10">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Etiqueta tono="lima">simulación acelerada</Etiqueta>
        </div>
      </div>
      ...
      <div className="mt-10 flex flex-wrap gap-3">
        {pedido.estado === 'sin_cobertura' || pedido.estado === 'cancelado' ? (
          <Boton variante="lima" onClick={() => reintentar(pedido.id)}>
            Volver a enviarlo
          </Boton>
        ) : null}
...
function Titular({ pedido, ahora }: { pedido: Pedido; ahora: number }) {
  ...
  const copia: Record<EstadoPedido, { titulo: string; texto: string }> = {
    buscando: {
      ...
      texto:
        pedido.modo === 'manual'
          ? 'La botillería tiene el pedido en pantalla y espera tu decisión. Ábrelo en el panel de botillería y tómalo.'
          : `La botillería está viendo tu pedido${
    ...
    sin_cobertura: {
      titulo: 'La botillería no pudo tomarlo',
      texto: 'No respondió a tiempo. Puedes reenviarlo y se le avisará de nuevo.',
    },
    cancelado: {
