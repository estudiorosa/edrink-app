...
export function TarjetaSugerencia({
  ...
  return (
    <article
      className={`entrada border bg-noche-alto/40 ${destacada ? 'border-lima/60' : 'border-noche-borde'}`}
    >
      <div className="grid gap-0 lg:grid-cols-[1fr_1fr]">
        <div className="border-b border-noche-borde p-6 sm:p-8 lg:border-r lg:border-b-0">
          <div className="flex flex-wrap items-center gap-2">
            {destacada ? <Etiqueta tono="lima">Mi primera opción</Etiqueta> : null}
          ...
          <ol className="mt-7 space-y-3 border-t border-noche-borde pt-5">
            {receta.pasos.map((paso, i) => (
              <li key={paso} className="flex gap-4 text-sm leading-relaxed text-bruma">
                <span className="numero shrink-0 text-lg text-lima">{i + 1}</span>
                <span>{paso}</span>
              </li>
            ))}
          </ol>

          <p className="mt-6 border-l-2 border-lima pl-4 text-sm leading-relaxed text-hueso">{receta.tip}</p>
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          {otras.length ? (
            <button
              ...
              className="mt-2 text-xs text-lima underline underline-offset-4 transition-colors hover:text-hueso"
            >
